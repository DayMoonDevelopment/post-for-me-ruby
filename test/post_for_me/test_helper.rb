...
class PostForMe::Test::SingletonClient < PostForMe::Client
  include Singleton

  TEST_API_BASE_URL = ENV.fetch("TEST_API_BASE_URL", "http://localhost:4010")

  def initialize
    super(base_url: PostForMe::Test::SingletonClient::TEST_API_BASE_URL, api_key: "My API Key")
  end
end
...
class PostForMe::Test::ResourceTest < Minitest::Test
  def async?
  ...
  def before_all
    super
    @post_for_me = PostForMe::Test::SingletonClient.instance
  end
